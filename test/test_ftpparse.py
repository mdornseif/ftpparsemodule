...
ftplines = [ \
            ...
            ('+i8388621.29609,m824255902,/,\tdev',
             [('dev', 0, 0, 824255902, 1, 1, 0, '8388621.29609', 1, 0)]),
            ('+i8388621.44468,m839956783,r,s10376,\tRFCEPLF',
             [('RFCEPLF', 10376, 1, 839956783, 1, 0, 1, '8388621.44468', 1, 0)]),
            ...
            ('dr-xr-xr-x   2 root     other        512 Apr  8  1994 etc',
             [('etc', 512, 1, 765763200, 3, 1, 0, None, 0, 0)]),
            ('dr-xr-xr-x   2 root     512 Apr  8  1994 etc',
             [('etc', 512, 1, 765763200, 3, 1, 0, None, 0, 0)]),
            ('-r-xr-xr-x   1 root  wheel  3258128 Nov 20  2000 kernel.GENERIC',
             [('kernel.GENERIC', 3258128, 1, 974678400, 3, 0, 1, None, 0, 0)]),
            ('lrwxrwxrwx   1 root  wheel       11 Feb 22  2001 sys -> usr/src/sys',
             [('sys', 11, 1, 982800000, 3, 1, 1, None, 0, 1)]),
            ('drwxr-xr-x   2 root  wheel      512 Feb 22  2001 cdrom',
             [('cdrom', 512, 1, 982800000, 3, 1, 0, None, 0, 0)]),
            #('lrwxrwxrwx   1 root     other          7 Jan 25 00:17 bin -> usr/bin',
            # [('bin', 7, 1, 980381820, 2, 1, 1, None, 0, 1)]),
            #('-rw-r-----  1 md  staff  13500 Dec 25 08:44 ftpparse.c',
            # [('ftpparse.c', 13500, 1, 1009269840, 2, 0, 1, None, 0, 0)]),
            # Also produced by Microsoft's FTP servers for Windows:
            #('----------   1 owner    group         1803128 Jul 10 10:18 ls-lR.Z',
            # [('ls-lR.Z', 1803128, 1, 994760280, 2, 0, 1, None, 0, 0)]),
            #('d---------   1 owner    group               0 May  9 19:45 Softlib',
            # [('Softlib', 0, 1, 989437500, 2, 1, 0, None, 0, 0)]),
            # Also WFTPD for MSDOS:
            ('-rwxrwxrwx   1 noone    nogroup      322 Aug 19  1996 message.ftp',
             [('message.ftp', 322, 1, 840412800, 3, 0, 1, None, 0, 0)]),
            # Also NetWare:
            #('d [R----F--] supervisor            512       Jan 16 18:53    login',
            # [('login', 512, 1, 979671180, 2, 1, 0, None, 0, 0)]),
            #('- [R----F--] rhesus             214059       Oct 20 15:27    cx.exe',
            # [('cx.exe', 214059, 1, 1003591620, 2, 0, 1, None, 0, 0)]),
            # Also NetPresenz for the Mac:
            ('-------r--         326  1391972  1392298 Nov 22  1995 MegaPhone.sit',
             [('MegaPhone.sit', 1392298, 1, 816998400, 3, 0, 1, None, 0, 0)]),
            ('drwxrwxr-x               folder        2 May 10  1996 network',
             [('network', 2, 1, 831686400, 3, 1, 0, None, 0, 0)]),
            # MultiNet (some spaces removed from examples)
            ('00README.TXT;1      2 30-DEC-1996 17:44 [SYSTEM] (RWED,RWED,RE,RE)',
             [('00README.TXT', 0, 0, 851967840, 2, 0, 1, None, 0, 0)]),
            ('CORE.DIR;1          1  8-SEP-1996 16:09 [SYSTEM] (RWE,RWE,RE,RE)',
             [('CORE', 0, 0, 842198940, 2, 1, 0, None, 0, 0)]),
            # and non-MutliNet VMS:
            ('CII-MANUAL.TEX;1  213/216  29-JAN-1996 03:33:12  [ANONYMOU,ANONYMOUS]   (RWED,RWED,,)',
             [('CII-MANUAL.TEX', 0, 0, 822886380, 2, 0, 1, None, 0, 0)]),
            # MSDOS format
            ('04-27-00  09:09PM       <DIR>          licensed',
             [('licensed', 0, 0, 956869740, 2, 1, 0, None, 0, 0)]),
            ('07-18-00  10:16AM       <DIR>          pub',
             [('pub', 0, 0, 963915360, 2, 1, 0, None, 0, 0)]),
            ('04-14-00  03:47PM                  589 readme.htm',
             [('readme.htm', 589, 1, 955727220, 2, 0, 1, None, 0, 0)]),
